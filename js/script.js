...
$(document).ready(function () {
    ...
    $('.contribuir').on('click', function () {
        $('#id_presente').val($(this).attr('value'));
        $('#confirmar_valor').show('slow');
        $('#lista_presentes').hide('slow');
    });

    $('#cc').on('click', function () {
        $.ajax({
            type: "POST",
            url: "/site/salvarContribuicao",
            data: {
                'nomec': $('#nomec').val(),
                'valorc': $('#valorc').val(),
                'idp': $('#id_presente').val()
    ...
    //===================================================================================
    var theday = new Date();
    theday = new Date(2017, 11, 23, 7, 0, 0);
