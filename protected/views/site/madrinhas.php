<head>
    <meta charset="utf-8">
    <meta content="width=device-width, initial-scale=1.0" name="viewport">
    <title>Lorrana & Pablo</title>

    <link rel="icon" href="/images/favicon.jpg">
    <!-- Bootstrap -->
    <link rel="stylesheet" href="/css/bootstrap/bootstrap.min.css" type="text/css" media="screen" />
    <!-- Pace -->
    <link rel="stylesheet" href="/css/preloader.css"  media="screen">
    <link rel="stylesheet" href="/css/preloader-default.css"  media="screen">
    <!-- Flexslider -->
    <link rel="stylesheet" href="/css/flexslider/flexslider.css" type="text/css">
    <!-- Animate -->
    <link rel="stylesheet" href="/css/animate/animate.css" type="text/css">       
    <!-- Countdown -->
    <link rel="stylesheet" href="/css/countdown/jquery.countdown.css" type="text/css">
    <!-- Magnific Popup -->
    <link rel="stylesheet" href="/css/magnific-popup/magnific-popup.css" type="text/css">
    <!-- Owl Carousel -->
    <link rel="stylesheet" href="/css/owlcarousel/owl.carousel.css" type="text/css">
    <link rel="stylesheet" href="/css/owlcarousel/owl.theme.css" type="text/css">
    <!-- Icon -->
    <link rel="stylesheet" href="/css/fonts/fontello/css/fontello.css" type="text/css" media="screen" />
    <!-- Font -->
    <link href='/fonts.googleapis.com/css3f04.css?family=Oswald' rel='stylesheet' type='text/css'>
    <link href='/fonts.googleapis.com/css9d89.css?family=Arvo:400,700' rel='stylesheet' type='text/css'>
    <link href='/fonts.googleapis.com/cssa7dc.css?family=Pacifico' rel='stylesheet' type='text/css'>

    <!-- Theme CSS -->
    <link href="/css/style.css" rel="stylesheet" media="screen">
</head>

<body class="no-slider-page">
    <audio autoplay="true" loop="true" src="/music/iy.mp3"></audio>
    <!--PRELOADER-->
    <div id="preloader">

        <div class="alignment">
            <div class="v-align center-middle"> 

                <!-- LEFT HEART -->
                <div class="heart-animation">                	
                    <i class="de-icon-heart"></i>
                </div>

                <!-- RIGHT HEART -->
                <div class="heart-animation-reverse">
                    <i class="de-icon-heart"></i>
                </div>     

            </div>
        </div>

    </div> <!--END of PRELOADER-->

    <!-- NAVIGATION -->    
    <header id="nav-header">
        <nav id="nav-bar" class="top-bar fluid-width block-color nav-center sticky-nav animation fadeInDown">

            <div id="nav-wrapper">

                <!-- LOGO -->
                <div class="logo-wrapper">
                    <!-- CSS LOGO --> 
                    <a href="/site/index">
                        <div class="css-logo rounded">
                            <div class="css-logo-text">
                                <strong>L</strong><i class="de-icon-heart-1"></i><strong>P</strong>
                            </div>
                        </div>
                    </a>

                    <!-- IMG LOGO 
                    <a href="index.html">
                            <div class="img-logo">
                                    <img src="/images/slide1.jpg">
                            </div>
                    </a>
                    -->
                </div>
                <!-- END of LOGO -->

                <div id="mobile-nav">
                    <i class="de-icon-menu"></i>
                </div>

                <ul id="nav-menu">
                    <!--<li class="first-child">
                        <a href="index.html">INICIO</a>
                    </li>-->
                    <li class="first-child">
                        <a href="#">SOBRE</a>
                        <ul>
                            <li>
                                <a href="/site/sobreEle">SOBRE ELE</a>
                            </li>
                            <li>
                                <a href="/site/sobreEla">SOBRE ELA</a>
                            </li>
                        </ul> 	
                    </li>
                    <li>
                        <a href="/site/galeria">GALERIA</a>
                        <!--<ul>
                            <li><a href="gallery.html">GALLERY</a></li>
                            <li><a href="gallery-slider.html">GALLERY WITH SLIDER</a></li>
                            <li><a href="gallery-full-5col.html">FULL GALLERY - 5 COLUMN</a></li>
                            <li><a href="gallery-full-4col.html">FULL GALLERY - 4 COLUMN</a></li>
                            <li><a href="gallery-full-3col.html">FULL GALLERY - 3 COLUMN</a></li>
                        </ul>-->
                    </li>
                    <li><a href="/site/historia">A HISTÓRIA</a></li>
                    <li class="first-child split-margin">
                        <a href="#">CASAMENTO</a>
                        <ul>
                            <li><a href="/site/local">LOCALIZAÇAO</a></li>
                            <li><a href="/site/local#map-canvas">COMO CHEGAR</a></li>
                            <li>
                                <a href="/site/madrinhas">MADRINHAS</a>
                            </li>
                            <li>
                                <a href="/site/padrinhos">PADRINHOS</a>
                            </li>
                        </ul>
                    </li>

                    <li>
                        <a href="/site/presentes">PRESENTES</a>
                    </li>
                    <!--<li>
                        <a href="page.html">PAGINA</a>
                    </li>-->
                    <li>
                        <a href="#">CONVIDADOS</a>
                        <ul>
                            <!--<li><a href="rsvp-1.html">RSVP - 1</a></li>
                            <li><a href="rsvp-2.html">RSVP - 2</a></li>
                            <li><a href="rsvp-3.html">RSVP - 3</a></li>-->
                            <li><a href="/site/presenca">CONFIRME SUA PRESENÇA</a></li>
                            <li><a href="/site/mensagens">NOS ENVIE UMA MENSAGEM</a></li>
                        </ul>
                    </li>
                </ul>

                <div class="clearboth"></div>

            </div>

        </nav>
    </header>
    <!-- END of NAVIGATION -->

    <!--CONTENT SECTION-->
    <section id="content">

        <!-- CONTAINER-1 -->
        <div class="container">            

            <!-- TITLE -->
            <div class="row">
                <div class="col-md-offset-3 col-md-6 text-center">

                    <div class="page-title">
                        <h1>MADRINHAS</h1>

                        <div class="heart-divider">
                            <span class="grey-line"></span>
                            <i class="de-icon-heart pink-heart"></i>
                            <i class="de-icon-heart grey-heart"></i>
                            <span class="grey-line"></span>
                        </div>
                        <p>
                            Eu juro, o pré requisito não foi a beleza...
                        </p>
                    </div>

                </div>
            </div><!-- END of TITLE -->                

        </div><!-- END of CONTAINER-1 -->


        <!-- CONTAINER-2 -->
        <div class="container">
            <div class="row">                                            
                <div class="col-md-offset-1 col-md-10">

                    <!-- CONTAINER-FLUID -->
                    <div class="container-fluid">

                        <!-- BRIDESMAID -->
                        <div class="row">

                            <!-- BRIDESMAID ITEM-1 -->
                            <!-- =================================================================== -->
                            <div class="bridesmaid-wrapper col-md-3 col-sm-3">

                                <!--PHOTO-ITEM-->
                                <div class="photo-item animation fadeIn">

                                    <!--PHOTO-->
                                    <img src="/images/mulheres/madrinha1.jpeg" alt="" class="hover-animation image-zoom-in">

                                    <!--PHOTO OVERLAY-->
                                    <div class="layer wh100 hidden-black-overlay hover-animation fade-in">
                                    </div><!--END of PHOTO OVERLAY-->

                                    <!--ICON LINK-->
                                    <div class="layer wh100 hidden-link hover-animation delay1 fade-in">
                                        <div class="alignment">
                                            <div class="v-align center-middle">

                                                <span class="de-button outline medium">
                                                    CLARA MACÊDO
                                                </span>

                                            </div>
                                        </div>
                                    </div><!--END of ICON LINK-->

                                </div><!--END of PHOTO-ITEM--> 

                                <!-- TITLE & EXCERPT -->
                                <div class="title-excerpt animation fadeIn text-center">
                                    <div class="de-icon circle very-small-size custom-heart-icon">
                                        <i class="de-icon-heart"></i>
                                    </div>
                                    <h4><a href="#">- PRIMA/AMIGA -</a></h4>
                                </div>
                                <!-- END of TITLE & EXCERPT -->

                            </div><!-- END of BRIDESMAID ITEM-1 -->


                            <!-- BRIDESMAID ITEM-2 -->
                            <!-- =================================================================== -->
                            <div class="bridesmaid-wrapper col-md-3 col-sm-3">

                                <!--PHOTO-ITEM-->
                                <div class="photo-item animation fadeIn">

                                    <!--PHOTO-->
                                    <img src="/images/mulheres/madrinha2.jpeg" alt="" class="hover-animation image-zoom-in">

                                    <!--PHOTO OVERLAY-->
                                    <div class="layer wh100 hidden-black-overlay hover-animation fade-in">
                                    </div><!--END of PHOTO OVERLAY-->

                                    <!--ICON LINK-->
                                    <div class="layer wh100 hidden-link hover-animation delay1 fade-in">
                                        <div class="alignment">
                                            <div class="v-align center-middle">

                                                <span class="de-button outline medium">
                                                    CECÍLIA GALDINO
                                                </span>

                                            </div>
                                        </div>
                                    </div><!--END of ICON LINK-->

                                </div><!--END of PHOTO-ITEM--> 

                                <!-- TITLE & EXCERPT -->
                                <div class="title-excerpt animation fadeIn text-center">
                                    <div class="de-icon circle very-small-size custom-heart-icon">
                                        <i class="de-icon-heart"></i>
                                    </div>
                                    <h4><a href="#">- FUTURA MAMÃE -</a></h4>
                                </div>
                                <!-- END of TITLE & EXCERPT -->

                            </div><!-- END of BRIDESMAID ITEM-2 -->


                            <!-- BRIDESMAID ITEM-3 -->
                            <!-- =================================================================== -->
                            <div class="bridesmaid-wrapper col-md-3 col-sm-3">

                                <!--PHOTO-ITEM-->
                                <div class="photo-item animation fadeIn">

                                    <!--PHOTO-->
                                    <img src="/images/mulheres/madrinha3.jpeg" alt="" class="hover-animation image-zoom-in">

                                    <!--PHOTO OVERLAY-->
                                    <div class="layer wh100 hidden-black-overlay hover-animation fade-in">
                                    </div><!--END of PHOTO OVERLAY-->

                                    <!--ICON LINK-->
                                    <div class="layer wh100 hidden-link hover-animation delay1 fade-in">
                                        <div class="alignment">
                                            <div class="v-align center-middle">

                                                <span class="de-button outline medium">
                                                    RAISSA ALBUQUERQUE
                                                </span>

                                            </div>
                                        </div>
                                    </div><!--END of ICON LINK-->

                                </div><!--END of PHOTO-ITEM--> 

                                <!-- TITLE & EXCERPT -->
                                <div class="title-excerpt animation fadeIn text-center">
                                    <div class="de-icon circle very-small-size custom-heart-icon">
                                        <i class="de-icon-heart"></i>
                                    </div>
                                    <h4><a href="#">- MELHOR AMIGA -</a></h4>
                                </div>
                                <!-- END of BRIDESMAID ITEM-4 -->
                                <!-- END of TITLE & EXCERPT -->

                            </div><!-- END of BRIDESMAID ITEM-3 -->
                            <div class="bridesmaid-wrapper col-md-3 col-sm-3">

                                    <!--PHOTO-ITEM-->
                                    <div class="photo-item animation fadeIn">

                                        <!--PHOTO-->
                                        <img src="/images/mulheres/madrinha4.jpeg" alt="" class="hover-animation image-zoom-in">

                                        <!--PHOTO OVERLAY-->
                                        <div class="layer wh100 hidden-black-overlay hover-animation fade-in">
                                        </div><!--END of PHOTO OVERLAY-->

                                        <!--ICON LINK-->
                                        <div class="layer wh100 hidden-link hover-animation delay1 fade-in">
                                            <div class="alignment">
                                                <div class="v-align center-middle">

                                                    <span class="de-button outline medium">
                                                        ISADORA FURTADO
                                                    </span>

                                                </div>
                                            </div>
                                        </div><!--END of ICON LINK-->

                                    </div><!--END of PHOTO-ITEM--> 

                                    <!-- TITLE & EXCERPT -->
                                    <div class="title-excerpt animation fadeIn text-center">
                                        <div class="de-icon circle very-small-size custom-heart-icon">
                                            <i class="de-icon-heart"></i>
                                        </div>
                                        <h4><a href="#">- DEUSA GREGA -</a></h4>
                                    </div>
                                    <!-- END of TITLE & EXCERPT -->

                                </div>


                            <!-- BRIDESMAID ITEM-4 -->
                            <!-- =================================================================== -->
                            
                            <!-- BRIDESMAID ITEM-5 -->
                            <!-- =================================================================== -->
                            <div class="bridesmaid-wrapper col-md-3 col-sm-3">

                                <!--PHOTO-ITEM-->
                                <div class="photo-item animation fadeIn">

                                    <!--PHOTO-->
                                    <img src="/images/mulheres/madrinha5.jpeg" alt="" class="hover-animation image-zoom-in">

                                    <!--PHOTO OVERLAY-->
                                    <div class="layer wh100 hidden-black-overlay hover-animation fade-in">
                                    </div><!--END of PHOTO OVERLAY-->

                                    <!--ICON LINK-->
                                    <div class="layer wh100 hidden-link hover-animation delay1 fade-in">
                                        <div class="alignment">
                                            <div class="v-align center-middle">

                                                <span class="de-button outline medium">
                                                    ROSA KELLY
                                                </span>

                                            </div>
                                        </div>
                                    </div><!--END of ICON LINK-->

                                </div><!--END of PHOTO-ITEM--> 

                                <!-- TITLE & EXCERPT -->
                                <div class="title-excerpt animation fadeIn text-center">
                                    <div class="de-icon circle very-small-size custom-heart-icon">
                                        <i class="de-icon-heart"></i>
                                    </div>
                                    <h4><a href="#">- MINHA FLOR -</a></h4>
                                </div>
                                <!-- END of TITLE & EXCERPT -->

                            </div><!-- END of BRIDESMAID ITEM-5 -->



                            <!-- BRIDESMAID ITEM-6 -->
                            <!-- =================================================================== -->
                            <div class="bridesmaid-wrapper col-md-3 col-sm-3">

                                <!--PHOTO-ITEM-->
                                <div class="photo-item animation fadeIn">

                                    <!--PHOTO-->
                                    <img src="/images/mulheres/madrinha6.jpeg" alt="" class="hover-animation image-zoom-in">

                                    <!--PHOTO OVERLAY-->
                                    <div class="layer wh100 hidden-black-overlay hover-animation fade-in">
                                    </div><!--END of PHOTO OVERLAY-->

                                    <!--ICON LINK-->
                                    <div class="layer wh100 hidden-link hover-animation delay1 fade-in">
                                        <div class="alignment">
                                            <div class="v-align center-middle">

                                                <span class="de-button outline medium">
                                                    GORETE MEDEIROS
                                                </span>

                                            </div>
                                        </div>
                                    </div><!--END of ICON LINK-->

                                </div><!--END of PHOTO-ITEM--> 

                                <!-- TITLE & EXCERPT -->
                                <div class="title-excerpt animation fadeIn text-center">
                                    <div class="de-icon circle very-small-size custom-heart-icon">
                                        <i class="de-icon-heart"></i>
                                    </div>
                                    <h4><a href="#">- 2º MÃE DA NOIVA -</a></h4>
                                </div>
                                <!-- END of TITLE & EXCERPT -->

                            </div><!-- END of BRIDESMAID ITEM-6 -->
                            <div class="bridesmaid-wrapper col-md-3 col-sm-3">

                                <!--PHOTO-ITEM-->
                                <div class="photo-item animation fadeIn">

                                    <!--PHOTO-->
                                    <img src="/images/mulheres/madrinha7.jpeg" alt="" class="hover-animation image-zoom-in">

                                    <!--PHOTO OVERLAY-->
                                    <div class="layer wh100 hidden-black-overlay hover-animation fade-in">
                                    </div><!--END of PHOTO OVERLAY-->

                                    <!--ICON LINK-->
                                    <div class="layer wh100 hidden-link hover-animation delay1 fade-in">
                                        <div class="alignment">
                                            <div class="v-align center-middle">

                                                <span class="de-button outline medium">
                                                    MARY CAMPOS
                                                </span>

                                            </div>
                                        </div>
                                    </div><!--END of ICON LINK-->

                                </div><!--END of PHOTO-ITEM--> 

                                <!-- TITLE & EXCERPT -->
                                <div class="title-excerpt animation fadeIn text-center">
                                    <div class="de-icon circle very-small-size custom-heart-icon">
                                        <i class="de-icon-heart"></i>
                                    </div>
                                    <h4><a href="#">- CONSELHEIRA -</a></h4>
                                </div>
                                <!-- END of TITLE & EXCERPT -->

                            </div>
                            <div class="bridesmaid-wrapper col-md-3 col-sm-3">

                                <!--PHOTO-ITEM-->
                                <div class="photo-item animation fadeIn">

                                    <!--PHOTO-->
                                    <img src="/images/mulheres/madrinha8.jpeg" alt="" class="hover-animation image-zoom-in">

                                    <!--PHOTO OVERLAY-->
                                    <div class="layer wh100 hidden-black-overlay hover-animation fade-in">
                                    </div><!--END of PHOTO OVERLAY-->

                                    <!--ICON LINK-->
                                    <div class="layer wh100 hidden-link hover-animation delay1 fade-in">
                                        <div class="alignment">
                                            <div class="v-align center-middle">

                                                <span class="de-button outline medium">
                                                    ANITA ALVES
                                                </span>

                                            </div>
                                        </div>
                                    </div><!--END of ICON LINK-->

                                </div><!--END of PHOTO-ITEM--> 

                                <!-- TITLE & EXCERPT -->
                                <div class="title-excerpt animation fadeIn text-center">
                                    <div class="de-icon circle very-small-size custom-heart-icon">
                                        <i class="de-icon-heart"></i>
                                    </div>
                                    <h4><a href="#">- 2º MÃE DO NOIVO -</a></h4>
                                </div>
                                <!-- END of TITLE & EXCERPT -->

                            </div>

                        </div> <!-- END of BRIDESMAID -->

                    </div><!-- END of CONTAINER FLUID -->

                </div>                     
            </div>                  
        </div><!-- END OF CONTAINER-2 -->

    </section><!-- END OF CONTENT SECTION -->

    <section id="content" style="margin-top: -150px;">

        <!-- CONTAINER-1 -->
        <div class="container">            

            <!-- TITLE -->
            <div class="row">
                <div class="col-md-offset-3 col-md-6 text-center">

                    <div class="page-title">
                        <h1>MADEMOISELLES</h1>

                        <div class="heart-divider">
                            <span class="grey-line"></span>
                            <i class="de-icon-heart pink-heart"></i>
                            <i class="de-icon-heart grey-heart"></i>
                            <span class="grey-line"></span>
                        </div>
                        <p>
                            ...aqui também não foi a beleza o pré requisito rsrs
                        </p>
                    </div>

                </div>
            </div><!-- END of TITLE -->                

        </div><!-- END of CONTAINER-1 -->


        <!-- CONTAINER-2 -->
        <div class="container">
            <div class="row">                                            
                <div class="col-md-offset-1 col-md-10">

                    <!-- CONTAINER-FLUID -->
                    <div class="container-fluid">

                        <!-- BRIDESMAID -->
                        <div class="row">

                            <!-- BRIDESMAID ITEM-1 -->
                            <!-- =================================================================== -->
                            <div class="bridesmaid-wrapper col-md-3 col-sm-3">

                                <!--PHOTO-ITEM-->
                                <div class="photo-item animation fadeIn">

                                    <!--PHOTO-->
                                    <img src="/images/mulheres/made1.jpeg" alt="" class="hover-animation image-zoom-in">

                                    <!--PHOTO OVERLAY-->
                                    <div class="layer wh100 hidden-black-overlay hover-animation fade-in">
                                    </div><!--END of PHOTO OVERLAY-->

                                    <!--ICON LINK-->
                                    <div class="layer wh100 hidden-link hover-animation delay1 fade-in">
                                        <div class="alignment">
                                            <div class="v-align center-middle">

                                                <span class="de-button outline medium">
                                                    LARISSA PALOMA
                                                </span>

                                            </div>
                                        </div>
                                    </div><!--END of ICON LINK-->

                                </div><!--END of PHOTO-ITEM--> 

                                <!-- TITLE & EXCERPT -->
                                <div class="title-excerpt animation fadeIn text-center">
                                    <div class="de-icon circle very-small-size custom-heart-icon">
                                        <i class="de-icon-heart"></i>
                                    </div>
                                    <h4><a href="#">- DO IFRN PRA VIDA -</a></h4>
                                </div>
                                <!-- END of TITLE & EXCERPT -->

                            </div><!-- END of BRIDESMAID ITEM-1 -->


                            <!-- BRIDESMAID ITEM-2 -->
                            <!-- =================================================================== -->
                            <div class="bridesmaid-wrapper col-md-3 col-sm-3">

                                <!--PHOTO-ITEM-->
                                <div class="photo-item animation fadeIn">

                                    <!--PHOTO-->
                                    <img src="/images/mulheres/made2.jpeg" alt="" class="hover-animation image-zoom-in">

                                    <!--PHOTO OVERLAY-->
                                    <div class="layer wh100 hidden-black-overlay hover-animation fade-in">
                                    </div><!--END of PHOTO OVERLAY-->

                                    <!--ICON LINK-->
                                    <div class="layer wh100 hidden-link hover-animation delay1 fade-in">
                                        <div class="alignment">
                                            <div class="v-align center-middle">

                                                <span class="de-button outline medium">
                                                    SANDELLY BEATRIZ
                                                </span>

                                            </div>
                                        </div>
                                    </div><!--END of ICON LINK-->

                                </div><!--END of PHOTO-ITEM--> 

                                <!-- TITLE & EXCERPT -->
                                <div class="title-excerpt animation fadeIn text-center">
                                    <div class="de-icon circle very-small-size custom-heart-icon">
                                        <i class="de-icon-heart"></i>
                                    </div>
                                    <h4><a href="#">- DESDE SEMPRE -</a></h4>
                                </div>
                                <!-- END of TITLE & EXCERPT -->

                            </div><!-- END of BRIDESMAID ITEM-2 -->


                            <!-- BRIDESMAID ITEM-3 -->
                            <!-- =================================================================== -->
                            <div class="bridesmaid-wrapper col-md-3 col-sm-3">

                                <!--PHOTO-ITEM-->
                                <div class="photo-item animation fadeIn">

                                    <!--PHOTO-->
                                    <img src="/images/mulheres/made3.jpeg" alt="" class="hover-animation image-zoom-in">

                                    <!--PHOTO OVERLAY-->
                                    <div class="layer wh100 hidden-black-overlay hover-animation fade-in">
                                    </div><!--END of PHOTO OVERLAY-->

                                    <!--ICON LINK-->
                                    <div class="layer wh100 hidden-link hover-animation delay1 fade-in">
                                        <div class="alignment">
                                            <div class="v-align center-middle">

                                                <span class="de-button outline medium">
                                                    ADYLLA ALEXYA
                                                </span>

                                            </div>
                                        </div>
                                    </div><!--END of ICON LINK-->

                                </div><!--END of PHOTO-ITEM--> 

                                <!-- TITLE & EXCERPT -->
                                <div class="title-excerpt animation fadeIn text-center">
                                    <div class="de-icon circle very-small-size custom-heart-icon">
                                        <i class="de-icon-heart"></i>
                                    </div>
                                    <h4><a href="#">- QUASE IRMÃ -</a></h4>
                                </div>
                                <!-- END of BRIDESMAID ITEM-4 -->
                                <!-- END of TITLE & EXCERPT -->

                            </div>
                            <div class="bridesmaid-wrapper col-md-3 col-sm-3">

                                <!--PHOTO-ITEM-->
                                <div class="photo-item animation fadeIn">

                                    <!--PHOTO-->
                                    <img src="/images/mulheres/made4.jpeg" alt="" class="hover-animation image-zoom-in">

                                    <!--PHOTO OVERLAY-->
                                    <div class="layer wh100 hidden-black-overlay hover-animation fade-in">
                                    </div><!--END of PHOTO OVERLAY-->

                                    <!--ICON LINK-->
                                    <div class="layer wh100 hidden-link hover-animation delay1 fade-in">
                                        <div class="alignment">
                                            <div class="v-align center-middle">

                                                <span class="de-button outline medium">
                                                    RUAMA ALBUQUERQUE
                                                </span>

                                            </div>
                                        </div>
                                    </div><!--END of ICON LINK-->

                                </div><!--END of PHOTO-ITEM--> 

                                <!-- TITLE & EXCERPT -->
                                <div class="title-excerpt animation fadeIn text-center">
                                    <div class="de-icon circle very-small-size custom-heart-icon">
                                        <i class="de-icon-heart"></i>
                                    </div>
                                    <h4><a href="#">- "MAIMOTENTA" -</a></h4>
                                </div>
                                <!-- END of BRIDESMAID ITEM-4 -->
                                <!-- END of TITLE & EXCERPT -->

                            </div><!-- END of BRIDESMAID ITEM-3 -->
                            

                    </div><!-- END of CONTAINER FLUID -->

                </div>                     
            </div>                  
        </div><!-- END OF CONTAINER-2 -->

    </section>

    <!-- FOOTER SECTION -->
    <footer>
        <div class="image-divider fixed-height" style="background-image:url(/images/lp/7.jpeg); height:400px" data-stellar-background-ratio="0.5" >

            <div class="divider-overlay"></div>

            <!-- Use Center Middle Alignment to align middle the content for fixed-height parallax -->
            <div class="alignment"> 
                <div class="v-align center-middle">                  	

                    <!-- HEART DIVIDER & COUPLE NAME -->
                    <div class="container">                 	 
                        <div class="row">
                            <div class="col-md-12">

                                <div class="heart-divider animation delay1 fadeInUp">
                                    <span class="white-line"></span>
                                    <i class="de-icon-heart pink-heart"></i>
                                    <i class="de-icon-heart white-heart"></i>
                                    <span class="white-line"></span>
                                </div>

                                <div id="footer-couple-name" class="animation delay1 fadeInUp">
                                    LORRANA & PABLO
                                </div>

                            </div>
                        </div>
                    </div><!-- END of HEART DIVIDER & COUPLE NAME -->

                </div>
            </div><!-- END of Center Middle Alignment -->

        </div>
    </footer><!-- END of FOOTER SECTION -->


    <!-- jQuery -->
    <script src="/js/jquery-1.11.1.min.js"></script>
    <!-- Pace -->
    <script src="/js/pace/pace.min.js"></script> 
    <!-- Bootstrap -->
    <script src="/js/bootstrap/bootstrap.js"></script>    
    <!-- Modernizr -->
    <script src="/js/modernizr/modernizr.js"></script>  
    <!-- Device JS -->
    <script src="/js/devicejs/device.js"></script>  
    <!-- TinyNav -->
    <script src="/js/tinynav/tinynav.min.js"></script>
    <!-- SmoothScroll -->
    <script src="/js/smoothscroll/jquery.smooth-scroll.js"></script>
    <!-- Flexslider -->
    <script src="/js/flexslider/jquery.flexslider.js"></script>  
    <!-- Sticky -->
    <script src="/js/sticky/jquery.sticky.js"></script>  
    <!-- Waypoint -->
    <script src="/js/waypoint/jquery.waypoints.min.js"></script>
    <!-- DoubleTapToGo -->
    <script src="/js/jquery-ui-widget/jquery.ui.widget.js"></script>
    <script src="/js/jquery-doubletaptogo/jquery.dcd.doubletaptogo.js"></script>
    <!-- Vide -->
    <script src="/js/vide/jquery.vide.js"></script>
    <!-- Stellar -->
    <script src="/js/stellar/jquery.stellar.js"></script>
    <!-- Masonry -->
    <script src="/js/masonry/masonry.pkgd.min.js"></script>
    <!-- Countdown -->
    <script src="/js/countdown/jquery.plugin.js"></script>
    <script src="/js/countdown/jquery.countdown.js"></script>
    <!-- Countdown Labels / Localisation -->
    <script src="/js/countdown/jquery.countdown-custom-label.js"></script>
    <!-- Magnific Popup -->
    <script src="/js/magnific-popup/jquery.magnific-popup.js"></script>
    <!-- Owl Carousel -->
    <script src="/js/owlcarousel/owl.carousel.js"></script>

    <!-- Custom Core Script -->
    <script type="text/javascript" src="/js/script.js"></script>        
    <!-- Custom Additional Script -->
    <script type="text/javascript" src="/js/main-slider-fade.js"></script>

</body>

<style type="text/css">
    div#content{
        padding: 0!important;
    }
</style>